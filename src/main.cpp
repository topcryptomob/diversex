...
// Copyright (c) 2017-2017 The DiverseX developers
...
#if defined(NDEBUG)
#error "DiverseX cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uxfe) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK())
        nSubsidy = 6000 * COIN; // 1.2M DVRSX Premine across 200 blocks
...
{
    RenameThread("diversex-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // DVRSX: We use certain sporks during IBD, so check to see if they are
